...
<script setup>
  ...
  const chartData = computed(() => {
    ...
    const filteredTags = props.tags.filter(tag => {
      ...
      return lowerName.includes('tolko') || lowerName.includes('biopappel')
    })
  ...
  const getDynamicTagClasses = tagsString => {
    ...
    if (lowerTags.includes('biopappel')) return `${baseClasses} bg-pluxeeGreen/10 text-pluxeeGreen border-pluxeeGreen/20`
