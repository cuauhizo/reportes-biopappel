        ...
        <div class="grid grid-cols-12 gap-2">
          <div class="col-span-12">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-2">
              ...
              <div class="bg-gray-50 p-6 rounded-xl shadow-sm">
                <p class="text-gray-500 text-sm font-bold">News Followers</p>
                <div class="flex items-baseline gap-3 mt-1 justify-start">
                  <h2 class="text-3xl font-black text-pluxeeBlue">{{ formatNumber(data.kpis.new_followers) }}</h2>
                  <!-- <span v-if="data.kpis.new_followers_diff" class="flex items-center rounded-full py-0.5 px-2 text-xs font-bold mb-0.5 gap-1" :class="data.kpis.new_followers_diff.increase ? 'bg-green-50 text-green-500' : 'bg-red-50 text-red-500'">
