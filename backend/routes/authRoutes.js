...
const { saveTokens, getTokens } = require('../utils/db')
...
// 🕵️‍♂️ RUTA TEMPORAL PARA DESCUBRIR LOS IDs DE HOOTSUITE
router.get('/mis-redes', async (req, res) => {
  try {
    const tokens = await getTokens()
    if (!tokens || !tokens.access_token) {
      return res.send('No hay token en MySQL. Ve a /api/auth/hootsuite para iniciar sesión primero.')
    }

    // Le pedimos a Hootsuite la lista completa de perfiles (sin pasarle ningún ID)
    const response = await axios.get('https://platform.hootsuite.com/v1/socialProfiles', {
      headers: { Authorization: `Bearer ${tokens.access_token}` },
    })

    // Te mostramos la respuesta en la pantalla
